...
import static org.jclouds.abiquo.domain.util.LinkUtils.getSelfLink;
...
/**
 * Base class for all Abiquo task types.
 * 
 * @author Ignasi Barrera
 */
public abstract class BaseTask<T> extends DomainWrapper<TaskDto> {

   protected BaseTask(ApiContext<AbiquoApi> context, TaskDto target) {
   ...
   @Override
   public void refresh() {
      RESTLink self = checkNotNull(getSelfLink(target), ValidationErrors.MISSING_REQUIRED_LINK + "self");
   ...
   /**
    * Get the result produced by the task.
    * 
   ...
   public NoResultTask asNoResultTask() {
      return NoResultTask.class.cast(this);
   }
